...
with col_title:
    st.markdown(
        ...
        <p style='font-size:16px;margin-top:4px;color:#9db2cf'>
            Cabin Crew Training & Development — Heston Airlines
        </p>
        """,
        unsafe_allow_html=True,
    )

st.caption("Upload your Excel exports, filter to your instructor team, and get instant trends, alerts, and exports.")
...
EXPECTED_HINTS = {
    "Submitted","Course","Instructor","Location",
...
def parse_dates(df, date_col):
    if not date_col:
        return df.assign(_date=pd.NaT, _datetime=pd.NaT, _year=np.nan, _month="", _quarter=""), None, None
...
def build_report_excel(dff, rating_cols, note_cols, instructor_col, threshold):
    ...
    with pd.ExcelWriter(buffer, engine="xlsxwriter") as writer:
        ov = pd.DataFrame([["Total forms", len(dff)],
        ...
        if rating_cols:
            ...
            overall_avgs = {}
        pd.DataFrame([{"Total forms": len(dff),
                       f"Forms≤{threshold}(any)": int(dff["_any_leT"].sum()),
                       **overall_avgs}]).to_excel(writer, sheet_name="Overall Averages", index=False)
        ...
        if "_year" in dff.columns and dff["_year"].notna().any():
            y_rows = []
            for (name, yr), g in dff.groupby([dff[instructor_col].fillna("Unknown"), "_year"], dropna=False):
                if pd.isna(yr): 
                    continue
                row = {"Instructor": name, "Year": int(yr),
                       "Forms": len(g),
                       f"Forms≤{threshold}(any)": int(g["_any_leT"].sum())}
                for c in rating_cols or []:
                    row[f"avg|{c}"] = round(g[c].mean(), 2)
        ...
        for _, r in dff.iterrows():
            ...
            for c in note_cols or []:
                ...
                if pd.notna(val) and str(val).strip():
                    texts.append(str(val).strip())
            if texts:
                comm_records.append({"Instructor": r[instructor_col],
...
with st.sidebar:
    ...
    nav = st.radio("Navigate", ["Dashboard", "Instructor Profiles"], horizontal=False)
...
if nav == "Dashboard":
            ...
            if len(qtr_avgs) >= 2:
                ...
                for metric, delta in diff.items():
                    if pd.isna(delta): 
                        continue
                    if delta <= -0.2:
                        alerts.append(f"🔴 {metric}: decreasing by {delta} vs previous quarter → review recommended.")
                    elif delta >= 0.2:
                        alerts.append(f"🟢 {metric}: improving by +{delta} vs previous quarter.")
            if alerts:
    ...
    for _, r in dff.iterrows():
        ...
        for c in note_cols or []:
            ...
            if pd.notna(val) and str(val).strip():
                texts.append(str(val).strip())
        if texts:
            comm_records.append({"Instructor": r[instructor_col], "LowMarksCount": int(r.get("_leT_count", 0)), "Comments": " | ".join(texts)})
    ...
    st.download_button("⬇️ Download Excel report", data=report_bytes, file_name=f"Feedback_Report_{datetime.now().strftime('%Y-%m-%d')}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
    if not comm_df.empty:
        st.download_button("⬇️ Download comments (CSV)", data=comm_df.to_csv(index=False).encode("utf-8"), file_name="comments.csv", mime="text/csv")
    ...
    st.download_button(f"⬇️ Download report for {pick}", data=exp_bytes, file_name=f"{pick.replace(',','').replace(' ','_')}_Report_{datetime.now().strftime('%Y-%m-%d')}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
