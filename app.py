...
from summary_export import generate_summary_pdf, generate_instructor_pdf
...
def render_dashboard(df_filtered: pd.DataFrame):
    ...
    # PDF Export
    with st.expander("Management report export"):
        if st.button("Download summary PDF"):
            pdf_buffer = io.BytesIO()
            generate_summary_pdf(pdf_buffer, kpis)
            pdf_buffer.seek(0)
            st.download_button(
                "Download PDF",
                data=pdf_buffer,
                file_name="instructor_performance_summary.pdf",
...
def render_instructor_profiles(df_filtered: pd.DataFrame):
    ...
    # PDF export for this instructor
    with st.expander("Export instructor report"):
        if st.button("Download instructor PDF"):
            pdf_buffer = io.BytesIO()
            generate_instructor_pdf(pdf_buffer, stats)
            pdf_buffer.seek(0)
            st.download_button(
                "Download PDF",
                data=pdf_buffer,
                file_name=f"instructor_report_{selected_instructor}.pdf",
                mime="application/pdf",
            )
